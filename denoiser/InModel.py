...
from .demucs import Demucs, BLSTM, rescale_module, downsample2, upsample2
from .utils import capture_init


class InEnhancer_2ch(nn.Module):
    """
...
class InEnhancer_rep(nn.Module):
    """
        Demucs speech enhancement model.
        Args:
            - chin (int): number of input channels.
...
class InEnhancer_selfrep(nn.Module):
    ...
    def forward(self, mix):
        ...
        for _ in range(self.rep_depth):
            result = self.relu(self.denoiser(result))
            #input = th.clone(result)
        return result
...
class U_net_plus(nn.Module):
    ...
    def __init__(self,
        ...
        for index in range(depth):
            ...
            if index > 0 :
                for _ in range(self.depth - index):
                    decode = []
                    decode += [
                        nn.Conv1d(hidden, ch_scale * hidden, 1), activation,
                        nn.ConvTranspose1d(hidden, chout, kernel_size, stride),
    ...
    def forward(self, mix):
        ...
        x = skips.pop(-1)
        index = 0
        for current in range(self.depth - 1):
            current_skips = []
...
class Dilated_denoiser(nn.Module):
    """
    Demucs speech enhancement model.
    Args:
        - chin (int): number of input channels.
    ...
    def __init__(self,
        ...
        for index in range(depth):
            encode = []
            encode += [
                nn.Conv1d(chin, hidden, kernel_size, stride, dilation=dilation**index),
                nn.ReLU(),
                nn.Conv1d(hidden, hidden * ch_scale, 1), activation,
            ]
            self.encoder.append(nn.Sequential(*encode))
            ...
            decode += [
                nn.Conv1d(hidden, ch_scale * hidden, 1), activation,
                nn.ConvTranspose1d(hidden, chout, kernel_size, stride, dilation=dilation**index),
            ]
            if index > 0:
                decode.append(nn.ReLU())
            self.decoder.insert(0, nn.Sequential(*decode))
    ...
    def valid_length(self, length):
        ...
        length = math.ceil(length * self.resample)
        for idx in range(self.depth):
            length = math.ceil((length - (self.dilation ** idx) * (self.kernel_size - 1) - 1) / self.stride) + 1
            length = max(length, 1)
        for idx in range(self.depth):
            length = (length - 1) * self.stride + (self.dilation ** (self.depth - 1 - idx)) * (self.kernel_size - 1) + 1
        length = int(math.ceil(length / self.resample))
        return int(length)
