...
class InEnhancer_conv(nn.Module):
    """
        Demucs speech enhancement model.
        Args:
            - chin (int): number of input channels.
    ...
    def __init__(self,
        ...
        if alternative:
            self.conv = nn.ConvTranspose1d(2, 1, kernel_size=1)
        else:
            self.conv = nn.ConvTranspose1d(2, 1, kernel_size=3, padding=1)
...
class ChannelUnify(nn.Module):
    def __init__(self, chin, chout):
        super(ChannelUnify, self).__init__()

        # initialize weights
        self.w = nn.Parameter(th.zeros(1, chout, chin))
        self.b = nn.Parameter(th.zeros(1, chout, 1))

        # change weights to kaiming
        self.reset_parameters(self.w, self.b)

    def reset_parameters(self, weights, bias):
        th.nn.init.kaiming_uniform_(weights, a=math.sqrt(3))
        fan_in, _ = th.nn.init._calculate_fan_in_and_fan_out(weights)
